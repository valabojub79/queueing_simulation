%implementation of a simple M/D/1
    ...
    while(num_clients_count < sim_packets)
        ...
        if (type_of_event == 1)
            ...
            if(server_status == 1)
                ...
                server_status = 1;
                time_next_event(2) = sim_time + service_mean_time;
...
%----------------------graphs--------------------------------
figure('name','Mean Number of Clients in Queue (M/D/1)');
plot(P,avg_num_in_queue,'r',P,P.*P./(2*(1-P)),'b');
legend('simulation','theoretical');
title('Mean Number of Clients in Queue (M/D/1)');
xlabel('P');
ylabel('mean number of clients');
axis([0 0.91 0 15]);

figure('name','Mean Delay in Queue (M/D/1)');
plot(P,avg_delay,'r',P,service_mean_time*P./(2*(1-P)),'b');
legend('simulation','theoretical');
title('Mean Delay in Queue (M/D/1)');
xlabel('P');
ylabel('mean delay (hrs)');
axis([0 0.91 0 0.15]);

figure('name', 'Utilisation (M/D/1)');
plot(P,util,'r',P,P,'b');
legend('simulation','theoretical');
title('Utilisation (M/D/1)');
xlabel('P');
ylabel('Utilisation');
axis([0 0.91 0 1]);
